...
namespace Microsoft.WindowsAzure
{
    ...
    /// <summary>Contains helper methods for implementing shared access signatures.</summary>
    internal static class SharedAccessSignatureHelper
    {
        ...
        /// <summary>Converts the specified value to either a string representation or <see cref="String.Empty"/> .</summary>
        ...
        {
            var result = GetDateTimeOrNull(value) ?? string.Empty;
            return result;
        }

        /// <summary>Converts the specified value to either a string representation or <c>null</c> .</summary>
        /// <param name="value">The value to convert. </param>
        /// <returns>A string representing the specified value. </returns>
        internal static string GetDateTimeOrNull(DateTime? value)
        {
            var result = value != null ? value.Value.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ") : null;
            return result;
        }
        ...
        /// <summary>Get the signature hash embedded inside the Shared Access Signature.</summary>
        /// <param name="policy">The shared access policy to hash. </param>
        /// <param name="groupPolicyIdentifier">An optional identifier for the policy. </param>
        ...
        internal static string GetSharedAccessSignatureHashImpl(
            SharedAccessPolicy policy, string groupPolicyIdentifier, string resourceName, CloudBlobClient client)
        {
            CommonUtils.AssertNotNull("policy", policy);
            ...
            var signature = client.Credentials.ComputeHmac(stringToSign);

            return signature;
        }

        /// <summary>Parses the query.</summary>
        /// <param name="queryParameters">The query parameters. </param>
        /// <param name="credentials">The credentials. </param>
        internal static void ParseQuery(
            NameValueCollection queryParameters, out StorageCredentialsSharedAccessSignature credentials)
        {
            ...
            foreach (var key in queryParameters.AllKeys)
            {
                switch (key.ToLower())
                {
                    ...
                    case Constants.QueryConstants.SignedIdentifierShort:
            ...
            if (sasParameterFound)
            {
                ...
                {
                    var errorMessage = string.Format(CultureInfo.CurrentCulture, SR.MissingMandatoryParamtersForSAS);
                    throw new ArgumentException(errorMessage);
                }
                ...
                AddEscapedIfNotNull(builder, Constants.QueryConstants.SignedIdentifierShort, signedIdentifier);

...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>This class provides MemoryStream-like behavior but uses a list of blocks rather than a single piece of bufferBlocks.</summary>
    internal class SmallBlockMemoryStream : Stream
    {
        #region Constants and Fields

        /// <summary>The default block size.</summary>
        private const long DefaultBlockSize = 1024 * 1024; // 1MB

        /// <summary>The size of the block.</summary>
        private readonly long blockSize;

        /// <summary>The underlying bufferBlocks for the stream.</summary>
        private readonly List<byte[]> bufferBlocks = new List<byte[]>();

        /// <summary>The total capacity of the stream.</summary>
        private long capacity;

        /// <summary>The currently used length.</summary>
        private long length;
        ...
        /// <summary>Initializes a new instance of the SmallBlockMemoryStream class with default 64KB block size and no reserved space.</summary>
        public SmallBlockMemoryStream()
            : this(DefaultBlockSize)
        {
        }

        /// <summary>Initializes a new instance of the SmallBlockMemoryStream class with provided block size and no reserved space.</summary>
        ...
        /// <summary>Initializes a new instance of the SmallBlockMemoryStream class.</summary>
        /// <param name="blockSize">The size of blocks to use. </param>
        ...
        public SmallBlockMemoryStream(long blockSize, long reservedSize)
        {
            if (blockSize <= 0)
            {
                throw new ArgumentOutOfRangeException("blockSize", SR.BlockSize_must_be_a_positive__non_zero_value);
            }
        ...
        /// <summary>Gets the currently written length.</summary>
        ...
        /// <summary>Represents the current position in the stream.</summary>
        ...
        /// <summary>Does not perform any operation as it's an in-memory stream.</summary>
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (offset < 0)
            {
                throw new ArgumentOutOfRangeException("offset", SR.Write_Offset_must_be_positive);
            }

            if (count < 0)
            {
                throw new ArgumentOutOfRangeException("count", SR.Count_must_be_positive);
            }

            if ((buffer.Length - offset) < count)
            {
                throw new ArgumentException(SR.Offset_and_count_are_beyond_the_buffer_size, "count");
            }
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            switch (origin)
            {
                case SeekOrigin.Begin:
                    if (offset < 0)
                    {
                        throw new ArgumentException(SR.Attempting_to_seek_before_the_start_of_the_stream, "origin");
                    }

                    if (offset > this.length)
                    {
                        throw new ArgumentException(SR.Attempting_to_seek_past_the_end_of_the_stream, "origin");
                    }
        ...
        /// <summary>Sets the length of the current stream. (preallocating the bufferBlocks).</summary>
        ...
        /// <exception cref="System.ArgumentNullException">Thrown if<paramref name="buffer"/>
        ///     is null.</exception>
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            if ((buffer.Length - offset) < count)
            {
                throw new ArgumentException(SR.SmallBlockMemoryStream_Write_Offset_subtracted_from_the_buffer_length_is_less_than_count, "offset");
            }
        ...
        /// <summary>Adds another block to the underlying bufferBlocks.</summary>
        ...
        /// <summary>Advances the current position of the stream and adjust the offset and remainder based on the amount completed.</summary>
        ...
        /// <summary>Calculate the block for the current position.</summary>
        ...
        private void GetCurrentBlock(out int blockPosition, out byte[] currentBlock)
        {
            ...
            var blockID = (int)(this.position / this.blockSize);
        ...
        private int ReadInternal(byte[] buffer, int offset, int count)
        {
            ...
            var readLength = (int)Math.Min(this.Length - this.Position, count);
            var leftToRead = readLength;
            ...
            while (leftToRead != 0)
            {
                ...
                // Copy the block
                var blockReadLength = Math.Min(leftToRead, currentBlock.Length - blockPosition);
        ...
        /// <summary>Ensures that the amount of bufferBlocks is greater than or equal to the required size. Does not trim the size.</summary>
        ...
        private void Reserve(long requiredSize)
        {
            if (requiredSize < 0)
            {
                throw new ArgumentOutOfRangeException("requiredSize", SR.SmallBlockMemoryStream_Reserve_The_size_must_be_positive);
            }
        ...
        /// <summary>Writes a sequence of bytes to the current stream and advances the current position within this stream by the number of bytes written. (Requires the stream to be of sufficient size for writing).</summary>
        /// <param name="buffer">An array of bytes. This method copies count bytes from buffer to the current stream. </param>
        /// <param name="offset">The zero-based byte offset in buffer at which to begin copying bytes to the current stream. </param>
        /// <param name="count">The number of bytes to be written to the current stream. </param>
        private void WriteInternal(byte[] buffer, int offset, int count)
        {
            while (count != 0)
            {
                ...
                // Copy the block
                var blockWriteLength = Math.Min(count, currentBlock.Length - blockPosition);

...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>The class is an append-only stream for writing into storage.</summary>
    internal class BlobWriteStream : BlobStream
    {
        #region Constants and Fields

        /// <summary>The set of options applying to the current blob.</summary>
        private readonly BlobRequestOptions currentModifier;

        /// <summary>The ongoing blob hash.</summary>
        private HashAlgorithm blobHash;

        /// <summary>A memory stream holding the current block information.</summary>
        private SmallBlockMemoryStream blockBuffer;

        /// <summary>The hash of the current block.</summary>
        private HashAlgorithm blockHash;

        /// <summary>Internal Block ID sequence number. Used in generating Block IDs.</summary>
        private long blockIdSequenceNumber = -1;

        /// <summary>The list of uploaded blocks.</summary>
        private List<string> blockList;

        /// <summary>The size of the blocks to use.</summary>
        private long blockSize;

        /// <summary>The stream is writable until committed/close.</summary>
            ...
            set
            {
                ...
                if (value > Constants.MaxBlockSize)
                {
                    throw new ArgumentOutOfRangeException(
                        "BlockSize", 
                        string.Format(CultureInfo.CurrentCulture, SR.BlockTooLargeError, Constants.MaxBlockSize));
                }
        ...
        /// <summary>Gets the current length (equal to the position).</summary>
        ...
        /// <summary>Gets a value indicating whether the upload is done in blocks.</summary>
        ...
        /// <summary>Aborts the upload of the blob.</summary>
        ...
        /// <summary>Begins an asynchronous operation to commit the blob.</summary>
        ...
        /// <exception cref="System.ArgumentNullException"><paramref name="buffer"/>
        ///     is null.</exception>
            ...
            byte[] buffer, int offset, int count, AsyncCallback callback, object state)
        {
            ...
            var task = this.UseBlocks
                           ? (Task<NullTaskReturn>)
                             new InvokeTaskSequenceTask(() => this.WriteBlockBlobImpl(buffer, offset, count))
                           : this.WriteNonBlockedBlobImpl(buffer, offset, count);

            return task.ToAsyncResult(callback, state);
        }

        /// <summary>Commits the blob on the server.</summary>
        public override void Commit()
        {
            TaskImplHelper.ExecuteImpl(this.CommitImpl);
        }

        /// <summary>Ends an asynchronous operation to commit the blob.</summary>
        ...
        /// <summary>B Ends an asynchronous write operation.</summary>
        /// <param name="asyncResult">An <see cref="IAsyncResult"/> that references the pending asynchronous operation. </param>
        /// <exception cref="ArgumentNullException">AsyncResult is null.</exception>
        ...
        /// <summary>Causes any buffered data to be written to the remote storage. If the blob is not using blocks, the blob is fully committed.</summary>
        ...
        /// <summary>The stream does not support setting of length.</summary>
        ...
        /// <summary>Write the provided data into the underlying stream.</summary>
        ...
        /// <summary>Implements the disposing logic of committing the blob.</summary>
        ...
        /// <summary>Verifies that the blob is in writable state.</summary>
        ...
        /// <summary>Commits the blob by uploading any remaining data and the blocklist asynchronously.</summary>
                    ...
                    () =>
                        {
                            ...
                            var putBlockList =
                                this.blockList.Select(id => new PutBlockListItem(id, BlockSearchMode.Uncommitted)).
                                    ToList();
        ...
        /// <summary>Creates the new block and the block hash.</summary>
        ...
        /// <summary>Implements the flushing sequence.</summary>
        ...
        private TaskSequence FlushInternal()
        {
            ...
            var newTask = this.UseBlocks
                              ? new InvokeTaskSequenceTask(this.UploadBlock)
                              : new InvokeTaskSequenceTask(this.UploadBlob);

            yield return newTask;
            var result = newTask.Result; // Materialize the results to ensure exception propagation
        }

        /// <summary>Generates a blockID using the block's hash and a prefix.</summary>
        /// <returns>The base64 encoded blockID. </returns>
        private string GetBlockID()
        {
            this.blockIdSequenceNumber += 1;
            return Utilities.GenerateBlockIDWithHash(
                StreamUtilities.GetHashValue(this.blockHash), this.blockIdSequenceNumber);
        }

        /// <summary>Resets the block and the block hash.</summary>
        ...
        /// <summary>Sets the MD5 of the blob.</summary>
        ...
        /// <summary>Implements a sequence of events to upload a full blob.</summary>
        ...
        /// <summary>Implements a sequence to upload a block.</summary>
        ...
        private TaskSequence UploadBlock()
        {
            ...
            var hash = Utilities.ExtractMD5ValueFromBlockID(blockID);
        ...
        /// <summary>Implements the block writing task.</summary>
        /// <param name="buffer">The buffer to write data from. </param>
        /// <param name="offset">The byte offset in buffer from which to begin writing. </param>
        /// <param name="count">The number of bytes to write. </param>
        ...
        private TaskSequence WriteBlockBlobImpl(byte[] buffer, int offset, int count)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    "count", string.Format(CultureInfo.CurrentCulture, SR.TooManyBlocksError, Constants.MaxBlockNumber));
            }
            ...
            while (count != 0)
            {
                ...
                var numCopied = (int)Math.Min(count, this.blockSize - this.blockBuffer.Length);
        ...
        /// <summary>Creates a task that writes data for a full blob.</summary>
        /// <param name="buffer">The buffer to write data from. </param>
        /// <param name="offset">The byte offset in buffer from which to begin writing. </param>
        /// <param name="count">The number of bytes to write. </param>
                ...
                () =>
                    {
                        ...
                        if (this.Length + count > Constants.MaxSingleUploadBlobSize)
                        {
                            throw new ArgumentOutOfRangeException(
                                "count", 
                                string.Format(
                                    CultureInfo.CurrentCulture, SR.BlobTooLargeError, Constants.MaxSingleUploadBlobSize));
                        }

                        this.blockBuffer.Write(buffer, offset, count);
                        this.position += count;
                        StreamUtilities.ComputeHash(buffer, offset, count, this.blockHash);
                        StreamUtilities.ComputeHash(buffer, offset, count, this.blobHash);
                            
                            // Update both, in case someone changes their mind mid-stream
                    });
        }

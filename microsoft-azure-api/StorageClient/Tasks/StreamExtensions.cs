...
namespace Microsoft.WindowsAzure.StorageClient.Tasks
{
    ...
    /// <summary>Provides extensions for the stream object to support asynchronous operations.</summary>
    internal static class StreamExtensions
    {
        #region Constants and Fields

        /// <summary>Buffer size needs to be page-aligned.</summary>
        ...
        /// <summary>Asynchronously reads data from a stream using BeginRead.</summary>
        ...
        internal static Task<int> ReadAsync(this Stream stream, byte[] buffer, int offset, int count)
        {
            return new APMTask<int>(
                (callback, st) => stream.BeginRead(buffer, offset, count, callback, st), stream.EndRead);
        }

        /// <summary>Reads asynchronously the entire content of the stream and returns it as a string using StreamReader.</summary>
        /// <param name="stream">The stream on which the method is called. </param>
        ...
        [DebuggerNonUserCode]
        internal static TaskSequence ReadToString(this Stream stream, Encoding encoding, Action<string> result)
        {
            using (var ms = new SmallBlockMemoryStream())
            {
                ...
                while (read != 0)
                {
                    ...
                    var count = stream.ReadAsync(buffer, 0, 1024);
                    yield return count;
        ...
        /// <summary>Asynchronously writes data from a stream using BeginWrite.</summary>
        /// <param name="stream">The stream on which the method is called. </param>
        ...
        /// <summary>Reads asynchronously the entire content of the stream and writes it to the given output stream.</summary>
        ...
        /// <summary>Reads asynchronously the entire content of the stream and writes it to the given output stream. Closes the output stream at the end.</summary>
        /// <param name="stream">The origin stream. </param>
        /// <param name="toStream">The destination stream. </param>
        /// <returns>The sequence that when invoked results in an asynchronous copy. </returns>
        [DebuggerNonUserCode]
        ...
        /// <summary>Reads synchronously the entire content of the stream and writes it to the given output stream.</summary>
        /// <param name="stream">The origin stream. </param>
        /// <param name="toStream">The destination stream. </param>
        [DebuggerNonUserCode]
        internal static void WriteToSync(this Stream stream, Stream toStream)
        {
            ...
            do
            {
                var buffer = new byte[BufferSize];
                readCount =
                    stream.EndRead(stream.BeginRead(buffer, 0, buffer.Length, null /* Callback */, null /* State */));

...
namespace Microsoft.WindowsAzure.StorageClient.Tasks
{
    ...
    /// <summary>A set of extension methods for table service classes.</summary>
    internal static class TableServiceExtensions
    {
        #region Public Methods and Operators

        /// <summary>Converts a DataServiceQuery execution into an APMTask.</summary>
        ...
        public static Task<IEnumerable<T>> ExecuteAsync<T>(this DataServiceQuery<T> query)
        {
            ...
            return queryTask;
#else
            return GetUnexpectedInternalClientErrorWrappedTask(Constants.TableWorkaroundTimeout, queryTask);
#endif
        }

        /// <summary>Converts the SaveChanges method into an APMTask.</summary>
        ...
        /// <summary>Converts the SaveChanges method to an APMTask.</summary>
        /// <param name="context">The target context. </param>
        ...
        public static Task<DataServiceResponse> SaveChangesAsync(
            this TableServiceContext context, SaveChangesOptions options)
        {
            var task =
                new APMTask<DataServiceResponse>(
                    (callback, state) => context.BeginSaveChanges(options, callback, state), context.EndSaveChanges);
#if false
            return task;
#else
            return GetUnexpectedInternalClientErrorWrappedTask(Constants.TableWorkaroundTimeout, task);
#endif
        }
        ...
        /// <summary>Gets the unexpected internal client error wrapped task.</summary>
        ...
        internal static Task<T> GetUnexpectedInternalClientErrorWrappedTask<T>(TimeSpan? timeout, Task<T> originalTask)
        {
            ...
            if (timeout.HasValue)
            {
                Task<T> wrappedTimeoutTask =
                    new InvokeTaskSequenceTask<T>(
                        setResult => GetUnexpectedInternalClientErrorTaskSequence(timeout.Value, setResult));
        ...
        /// <summary>Gets the unexpected internal client error task sequence.</summary>
        /// <typeparam name="T">The result type. </typeparam>
        /// <param name="timeout">The timeout. </param>
        /// <param name="setResult">The set result. </param>
        /// <returns>A <see cref="TaskSequence"/> for the unexpected error. </returns>
        private static TaskSequence GetUnexpectedInternalClientErrorTaskSequence<T>(
            TimeSpan timeout, Action<T> setResult)
        {

...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>Represents a query against the Windows Azure Table service.</summary>
    ...
    public class CloudTableQuery<TElement> : IQueryable<TElement>
    {
        ...
        /// <summary>Stores the maximum results the table service can return.</summary>
        internal const int TableServiceMaxResults = 1000;

        /// <summary>Stores the header suffix for the next partition key.</summary>
        internal const string TableServiceNextPartitionKey = "NextPartitionKey";

        /// <summary>Stores the header suffix for the next row key.</summary>
        internal const string TableServiceNextRowKey = "NextRowKey";

        /// <summary>Stores the table suffix for the next table name.</summary>
        internal const string TableServiceNextTableName = "NextTableName";

        /// <summary>Stores the header prefix for continuation information.</summary>
        internal const string TableServicePrefixForTableContinuation = "x-ms-continuation-";

        /// <summary>Stores the pagination options.</summary>
        private readonly ResultPagination pagination;

        /// <summary>Stores the wrapped <see cref="DataServiceQuery{TElement}"/> .</summary>
        private readonly DataServiceQuery<TElement> query;

        #endregion

        #region Constructors and Destructors

        /// <summary>Initializes a new instance of the <see cref="CloudTableQuery{TElement}"/> class with the specified query.</summary>
        ...
        /// <summary>Initializes a new instance of the <see cref="CloudTableQuery{TElement}"/> class with the specified query and retry policy.</summary>
        /// <param name="query">The base query. </param>
        ...
        public CloudTableQuery(DataServiceQuery<TElement> query, RetryPolicy policy)
        {
            ...
            var queryTakeCount = GetQueryTakeCount(this.query);
        ...
        /// <value>The <see cref="T:System.Linq.Expressions.Expression"/> that is associated with this instance of <see cref="T:System.Linq.IQueryable"/> . </value>
        ...
        /// <summary>Gets or sets the retry policy for the query.</summary>
        ...
        /// <summary>Begins an asynchronous operation to execute a query and return the results as a result segment.</summary>
        ...
        /// <param name="state">A user-defined object that will be passed to the callback delegate. </param>
        /// <returns>An <see cref="IAsyncResult"/> that references the asynchronous operation. </returns>
        public IAsyncResult BeginExecuteSegmented(
            ResultContinuation continuationToken, AsyncCallback callback, object state)
        {
            return
                TaskImplHelper.BeginImplWithRetry<ResultSegment<TElement>>(
        ...
        /// <summary>Ends an asynchronous operation to execute a query and return the results as a result segment.</summary>
        ...
        /// <summary>Executes the query with the retry policy specified on the <see cref="CloudTableQuery&lt;TElement&gt;"/> object.</summary>
        ...
        public IEnumerable<TElement> Execute()
        {
            return CommonUtils.LazyEnumerateSegmented<TElement>(
                setResult => this.ExecuteSegmentedImpl(null, setResult), this.RetryPolicy);
        }
        ...
        {
            return
                CommonUtils.LazyEnumerateSegmented<TElement>(
                    setResult => this.ExecuteSegmentedImpl(continuationToken, setResult), this.RetryPolicy);
        }

        /// <summary>Expands the specified path.</summary>
        /// <param name="path">The path to expand. </param>
        /// <returns>A new query with the expanded path. </returns>
        public CloudTableQuery<TElement> Expand(string path)
        {
            return new CloudTableQuery<TElement>(this.query.Expand(path), this.RetryPolicy);
        }
        ...
        /// <returns>An <see cref="T:System.Collections.IEnumerator"/> that can be used to iterate through a collection. </returns>
        ...
        /// <summary>Executes the segmented impl.</summary>
        ...
        internal TaskSequence ExecuteSegmentedImpl(
            ResultContinuation continuationToken, Action<ResultSegment<TElement>> setResult)
        {
            ...
            try
            {
                ...
                var newContinuationToken = GetTableContinuationFromResponse(response);

                var listResult = new List<TElement>(responseResult.ToList());
                ...
                ResultSegment.CreateResultSegment(
                    ...
                    this.pagination, 
                    this.RetryPolicy, 
                    (scratch, continuationArg, setResultInnerArg) =>
                    this.ExecuteSegmentedImpl(continuationArg, setResultInnerArg));
            }
        ...
        /// <summary>Applies the continuation to query.</summary>
        /// <param name="continuationToken">The continuation token. </param>
        /// <param name="localQuery">The local query. </param>
        /// <returns>The modified query. </returns>
        private static DataServiceQuery<TElement> ApplyContinuationToQuery(
            ResultContinuation continuationToken, DataServiceQuery<TElement> localQuery)
        {
            if (continuationToken != null)
            {
                if (continuationToken.NextPartitionKey != null)
                {
                    localQuery = localQuery.AddQueryOption(
                        TableServiceNextPartitionKey, continuationToken.NextPartitionKey);
                }
        ...
        /// <summary>Gets the query take count.</summary>
        /// <param name="query">The query. </param>
        /// <returns>The take count of the query, if any. </returns>
        private static int? GetQueryTakeCount(IQueryable query)
        {
        ...
        /// <summary>Rewrites the query for take count.</summary>
        /// <param name="localQuery">The local query. </param>
        /// <param name="pagination">The pagination. </param>
        /// <returns>The rewritten query. </returns>
        private static DataServiceQuery<TElement> RewriteQueryForTakeCount(
            DataServiceQuery<TElement> localQuery, ResultPagination pagination)
        {
            var nextRequestPageSize = pagination.GetNextRequestPageSize();
                 ...
                 || pagination.EffectivePageSize > TableServiceMaxResults))
            {
                ...
                var adjustedTakeExpression = Expression.Call(
                    ...
                    Expression.Constant(Math.Min(nextRequestPageSize.GetValueOrDefault(), TableServiceMaxResults)));
        ...
        /// <summary>Gets the table continuation from response.</summary>
        /// <param name="response">The response. </param>
        /// <returns>The continuation. </returns>
        private static ResultContinuation GetTableContinuationFromResponse(OperationResponse response)
        {
            ...
            response.Headers.TryGetValue(
                TableServicePrefixForTableContinuation + TableServiceNextPartitionKey, out nextPartitionKey);
            response.Headers.TryGetValue(
                TableServicePrefixForTableContinuation + TableServiceNextRowKey, out nextRowKey);
            response.Headers.TryGetValue(
                TableServicePrefixForTableContinuation + TableServiceNextTableName, out nextTableName);

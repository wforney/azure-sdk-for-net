...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>General purpose utility methods.</summary>
    internal static class Utilities
    {
        #region Constants and Fields

        /// <summary>This is the limit where we allow for the error message returned by the server. Messages longer than that will be truncated.</summary>
        ...
        /// <summary>Copies the stream to buffer.</summary>
        ...
        /// <summary>Extracts an MD5 value from either an v1 or v2 block id.</summary>
        ...
        internal static string ExtractMD5ValueFromBlockID(string blockId)
        {
            ...
            if (blockId.Length == Constants.V1MD5BlockIdExpectedLength && blockId.StartsWith(Constants.V1BlockPrefix))
            {
        ...
        /// <summary>Generates an block id using the given MD5 hash value. The id must be Base64 compatible.</summary>
        ...
        internal static string GenerateBlockIDWithHash(string hashVal, long seqNo)
        {
            ...
            // WLF: REVIEW: Why is this needed here? It's in the return already.
            // ReSharper disable ReturnValueOfPureMethodIsNotUsed
            Convert.ToBase64String(tempArray);

            // ReSharper restore ReturnValueOfPureMethodIsNotUsed
            return string.Format(Constants.V2MD5BlockIdFormat, Convert.ToBase64String(tempArray), hashVal);
        }

        /// <summary>Gets the extended error details from response.</summary>
        ...
        internal static StorageExtendedErrorInformation GetExtendedErrorDetailsFromResponse(
            Stream httpResponseStream, long contentLength)
        {
            try
            {
                if (contentLength < 0)
                ...
                var bytesToRead = (int)Math.Min(contentLength, ErrorTextSizeLimit);
                var responseBuffer = new byte[bytesToRead];
                var bytesRead = CopyStreamToBuffer(httpResponseStream, responseBuffer, bytesToRead);
        ...
        /// <summary>Gets the extended error from XML message.</summary>
        ...
        internal static StorageExtendedErrorInformation GetExtendedErrorFromXmlMessage(string xmlErrorMessage)
        {
            ...
            var errorCodeName = XName.Get(Constants.TableErrorCodeElement, Constants.DataWebMetadataNamespace);
            var messageName = XName.Get(Constants.TableErrorMessageElement, Constants.DataWebMetadataNamespace);
            ...
            using (var reader = new StringReader(xmlErrorMessage))
            {
                ...
                var errorCodeElement = xDocument.Descendants(errorCodeName).FirstOrDefault();
                ...
                var messageElement = xDocument.Descendants(messageName).FirstOrDefault();
            ...
            var errorDetails = new StorageExtendedErrorInformation { ErrorMessage = message, ErrorCode = errorCode };
            return errorDetails;
        }

        /// <summary>Processes the unexpected status code.</summary>
        /// <param name="response">The response. </param>
        internal static void ProcessUnexpectedStatusCode(HttpWebResponse response)
        {
            throw new StorageServerException(
                StorageErrorCode.ServiceBadResponse, response.StatusDescription, response.StatusCode, null);
        }

        /// <summary>Translates the data service client exception.</summary>
        ...
        internal static Exception TranslateDataServiceClientException(InvalidOperationException e)
        {
            ...
            return TranslateFromHttpStatus(
                (HttpStatusCode)dsce.StatusCode, null, GetExtendedErrorFromXmlMessage(dsce.Message), e);
        }
            ...
            switch (statusCode)
            {
                case HttpStatusCode.Forbidden:
                    return new StorageClientException(
                        StorageErrorCode.AccessDenied, statusDescription, HttpStatusCode.Forbidden, details, inner);

                case HttpStatusCode.Gone:
                case HttpStatusCode.NotFound:
                    return new StorageClientException(
                        StorageErrorCode.ResourceNotFound, statusDescription, statusCode, details, inner);

                case HttpStatusCode.BadRequest:
                    return new StorageClientException(
                        StorageErrorCode.BadRequest, statusDescription, statusCode, details, inner);

                case HttpStatusCode.PreconditionFailed:
                case HttpStatusCode.NotModified:
                    return new StorageClientException(
                        StorageErrorCode.ConditionFailed, statusDescription, statusCode, details, inner);

                case HttpStatusCode.Conflict:
                    return new StorageClientException(
                        StorageErrorCode.ResourceAlreadyExists, statusDescription, statusCode, details, inner);

                case HttpStatusCode.GatewayTimeout:
                    return new StorageServerException(
                        StorageErrorCode.ServiceTimeout, statusDescription, statusCode, details, inner);
                ...
                case HttpStatusCode.InternalServerError:
                    return new StorageServerException(
                        StorageErrorCode.ServiceInternalError, statusDescription, statusCode, details, inner);

                case HttpStatusCode.NotImplemented:
                    return new StorageServerException(
                        StorageErrorCode.NotImplemented, statusDescription, statusCode, details, inner);

                case HttpStatusCode.BadGateway:
                    return new StorageServerException(
                        StorageErrorCode.BadGateway, statusDescription, statusCode, details, inner);

                case HttpStatusCode.HttpVersionNotSupported:
                    return new StorageServerException(
                        StorageErrorCode.HttpVersionNotSupported, statusDescription, statusCode, details, inner);
            }

            return null;
        }

        /// <summary>Translates the web exception.</summary>
        /// <param name="e">The exception. </param>
        /// <returns>The translated exception. </returns>
        internal static Exception TranslateWebException(Exception e)
        {
            ...
            if (we.Response != null)
            {
                ...
                var extendedError = GetExtendedErrorDetailsFromResponse(
                    response.GetResponseStream(), response.ContentLength);
                Exception translatedException;
                if (extendedError != null)
                {
                    translatedException = TranslateExtendedError(
                        extendedError, response.StatusCode, response.StatusDescription, e);
                ...
                translatedException = TranslateFromHttpStatus(
                    response.StatusCode, response.StatusDescription, extendedError, we);
            ...
            try
            {
                using (var reader = XmlReader.Create(inputStream))
                {
                    ...
                    do
                    {
                        ...
                        {
                            if (string.Compare(reader.LocalName, Constants.ErrorException, StringComparison.Ordinal)
                                == 0)
                            {
        ...
        /// <summary>Translates the extended error.</summary>
        /// <param name="details">The details. </param>
        /// <param name="statusCode">The status code. </param>
        /// <param name="statusDescription">The status description. </param>
        ...
        private static Exception TranslateExtendedError(
            ...
            Exception inner)
        {
            var errorCode = default(StorageErrorCode);
            ...
            {
                return new StorageClientException(errorCode, statusDescription, statusCode, details, inner);
            }
            ...
            if (errorCode != default(StorageErrorCode))
            {
                return new StorageServerException(errorCode, statusDescription, statusCode, details, inner);
            }

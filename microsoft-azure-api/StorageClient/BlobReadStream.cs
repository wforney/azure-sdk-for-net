...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>This class represents a seekable, read-only stream on a blob.</summary>
    internal class BlobReadStream : BlobStream
    {
        #region Constants and Fields

        /// <summary>The threshold beyond which we start a new read-Ahead.</summary>
        private const double ReadAheadThreshold = 0.25;

        /// <summary>The already available blocks for reading. This member is the only possible point of thread contention between the user's requests and ReadAhead async work. At any particular time, the ReadAhead thread may be adding more items into the list. The second thread will never remove/modify an existing item within the list.</summary>
        private readonly DownloadedBlockCollection downloadedBlocksList;

        /// <summary>The options applied to the stream.</summary>
        private readonly BlobRequestOptions options;

        /// <summary>The list of blocks for this blob.</summary>
        private List<ListBlockItem> blockList;

        /// <summary>The current position with the stream.</summary>
        private long position;

        /// <summary>A handle to the parallel download of data for ReadAhead.</summary>
        private IAsyncResult readAheadResult;

        /// <summary>The number of bytes to read forward on every request.</summary>
        private long readAheadSize;

        /// <summary>True if the AccessCondition has been changed to match a single ETag.</summary>
        ...
        /// <summary>Gets a value indicating whether the signature of each downloaded block should be verified.</summary>
        ...
        /// <value>The integrity control verification enabled.</value>
        public override sealed bool IntegrityControlVerificationEnabled { get; internal set; }

        /// <summary>Gets the length of the blob.</summary>
        ...
        /// <summary>Gets or sets the number of bytes to read ahead.</summary>
        /// <value>The read ahead size.</value>
        public override sealed long ReadAheadSize
        {
            get
            {
        ...
        /// <summary>Gets the number of bytes that are cached locally but not yet read by user.</summary>
        /// <value>The buffered data length.</value>
        ...
        /// <summary>Gets a value indicating whether length is available.</summary>
        /// <value>The length available.</value>
        private bool LengthAvailable
        {
            get
            {
                return (!string.IsNullOrEmpty(this.Blob.Properties.ETag) && (this.Blob.Properties.Length > 0))
                       || this.setEtagCondition;
            }
        }
        ...
        {
            return TaskImplHelper.BeginImpl<int>(
                result => this.ReadImplWrapper(buffer, offset, count, result), callback, state);
        }

        /// <summary>Ends an asynchronous read operation.</summary>
        ...
        ///     did not originate from a<see cref="M:System.IO.Stream.BeginRead(System.Byte[],System.Int32,System.Int32,System.AsyncCallback,System.Object)"/>
        ...
        /// <summary>Flush is not supported on read-only stream.</summary>
        public override void Flush()
        {
            throw new NotSupportedException();
        }

        /// <summary>Copies the specified amount of data from internal buffers to the buffer and advances the position.</summary>
        ...
        {
            return
                TaskImplHelper.ExecuteImpl<int>(
                    result => this.ReadImplWrapper(buffer, offset, count, result));
        }

        /// <summary>Seeks to the desired position. Any seek outside of the buffered/read-ahead data will cancel read-ahead and clear the buffered data.</summary>
        ...
        /// <summary>Setting the length of the blob is not supported.</summary>
        ...
        /// <summary>Write is not supported.</summary>
        ...
        /// <summary>Calculates if the currently held data is less than <see cref="ReadAheadThreshold"/> percentage depleted.</summary>
        ...
        /// <summary>Calculates the ReadAhead bounds (start and count) as required by the existing data.</summary>
        ...
        private void CalculateReadAheadBounds(
            long gapStart, long gapEnd, int count, out long startReadAhead, out long readAheadCount)
        {
            ...
            if (this.IntegrityControlVerificationEnabled && this.Blob.Properties.BlobType == BlobType.BlockBlob)
            {
                ...
                {
                    var blockEnd = blockStart + block.Size;

                    // If the start hasn't been set, calculate it
                    if (startReadAhead == -1)
                    {
                        if (gapStart >= blockStart && gapStart < blockEnd)
                        {
                    ...
                    if (startReadAhead != -1)
                    {
                        ...
                        if (desiredEnd >= blockStart && desiredEnd < blockEnd)
                        {
                            readAheadCount = blockEnd - startReadAhead;
                            break;
                        }
        ...
        /// <summary>Verifies if the given offset is within the bounds of the stream.</summary>
        ...
        /// <summary>Ends an asynchronous read-ahead operation.</summary>
        ...
        /// <summary>Locks download to a specific ETag.</summary>
        private void LockToEtag()
        {
            ...
            if (this.setEtagCondition == false && !string.IsNullOrEmpty(this.Blob.Properties.ETag))
            {
                ...
                {
                    if (
                        !this.options.AccessCondition.VerifyConditionHolds(
                            this.Blob.Properties.ETag, this.Blob.Properties.LastModifiedUtc))
                    {
        ...
        /// <summary>Reads the data from the service starting at the specified location. Verifies the block's signature (if required) and adds it to the buffered data.</summary>
        ...
        private TaskSequence ReadAheadImpl(long startPosition, long length)
        {
            ...
            var webResponseTask =
                new InvokeTaskSequenceTask<Stream>(
                    result => this.Blob.GetStreamImpl(this.options, position1, length, result));
            ...
            using (var stream = webResponseTask.Result)
            {
                ...
                if (this.IntegrityControlVerificationEnabled && this.Blob.Properties.BlobType == BlobType.BlockBlob)
                {
                    ...
                    foreach (var block in this.blockList)
                    {
                        ...
                        {
                            // Verify the hash
                            var blockNameMD5Value = Utilities.ExtractMD5ValueFromBlockID(block.Name);
                ...
                {
                    var memoryStream = new SmallBlockMemoryStream(Constants.DefaultBufferSize);
                    var copyTask = new InvokeTaskSequenceTask(() => stream.WriteTo(memoryStream));
        ...
        /// <summary>Reads from the verified blocks as much data as is available and needed.</summary>
        /// <param name="buffer">The buffer to read the data into. </param>
        /// <param name="offset">The zero-based byte offset in buffer at which to begin storing the data read from the current stream. </param>
        /// <param name="count">The maximum number of bytes to be read from the current stream. </param>
        ...
        /// <remarks>If verification is on, retrieve the blocklist. If there are downloaded blocks, read from there. Otherwise, if there's an outstanding request, wait for completion and read from there. Otherwise perform a new request.</remarks>
        private TaskSequence ReadImpl(byte[] buffer, int offset, int count, Action<int> setResult)
        {
            ...
            if (this.Blob.Properties.BlobType == BlobType.BlockBlob && this.IntegrityControlVerificationEnabled
                && this.blockList == null)
            {
                var blockListTask =
                    TaskImplHelper.GetRetryableAsyncTask<IEnumerable<ListBlockItem>>(
                        result => this.Blob.ToBlockBlob.GetDownloadBlockList(
                            BlockListingFilter.Committed, this.options, result),
            ...
            this.downloadedBlocksList.CalculateGapLength(
                this.position, count + this.readAheadSize, out gapStart, out gapEnd);
            ...
            if (this.CalculateIfBufferTooLow(gapStart) || !readComplete)
            {
                ...
                var outsideBounds = this.LengthAvailable
                                    && (this.Position >= this.Length || startReadAhead >= this.Length);
                ...
                if (readComplete)
                {
                    ...
                    if (this.readAheadResult == null)
                    {
                        this.readAheadResult =
                            TaskImplHelper.BeginImplWithRetry(
                                () => this.ReadAheadImpl(startReadAhead, readAheadCount),
                                this.options.RetryPolicy,
                                this.EndReadAhead,
                                null);
                    }
                ...
                {
                    var task =
                        TaskImplHelper.GetRetryableAsyncTask(
                            () => this.ReadAheadImpl(startReadAhead, readAheadCount), this.options.RetryPolicy);
                    ...
                    // readComplete =
                    this.ReadBufferedData(buffer, ref offset, ref count);
                }
            }
        ...
        /// <summary>Wraps the Read operation to remap any exceptions into IOException.</summary>
        /// <param name="buffer">The buffer to read the data into. </param>
        /// <param name="offset">The zero-based byte offset in buffer at which to begin storing the data read from the current stream. </param>
        /// <param name="count">The maximum number of bytes to be read from the current stream. </param>
        /// <param name="setResult">The action to be done upon completion to return the number of bytes read. </param>
        ...
        /// <summary>Retrieves the size of the blob.</summary>
        ...
        /// <summary>Verifies if the blocks are in expected format. Disables the integrity control if they are not appropriate for validation.</summary>
        private void VerifyBlocks()
        {
            ...
            else if (this.blockList[0].Name.Length == Constants.V1MD5BlockIdExpectedLength
                     && this.blockList[0].Name.StartsWith(Constants.V1BlockPrefix))
            {
                // Old 1.3 and lower style Block IDs
                foreach (var block in this.blockList)
                {
                    if (block.Name.Length != Constants.V1MD5BlockIdExpectedLength
                        || !block.Name.StartsWith(Constants.V1BlockPrefix))
                    {
                        this.IntegrityControlVerificationEnabled = false;
                        break;
                    }
        ...
        /// <summary>Represents a single block of data that was downloaded.</summary>
        internal class DownloadedBlock
        {
            #region Constructors and Destructors

            /// <summary>Initializes a new instance of the <see cref="DownloadedBlock"/> class.</summary>
            ...
            /// <summary>Gets the content of the block.</summary>
            /// <value>The block content.</value>
            public Stream BlockContent { get; private set; }

            /// <summary>Gets the starting location of the block.</summary>
        ...
        /// <summary>Encapsulates the collection of downloaded blocks and provides appropriate locking mechanism.</summary>
        internal class DownloadedBlockCollection
        {
            #region Constants and Fields

            /// <summary>Holds the downloaded blocks.</summary>
            private readonly SortedList<long, DownloadedBlock> downloadedBlocksList =
                new SortedList<long, DownloadedBlock>();

            #endregion

            #region Methods

            /// <summary>Adds the specified downloaded block.</summary>
            ...
            /// <summary>Tells if there are any blocks available.</summary>
            ...
            /// <summary>Calculates the length of the gap relative to <paramref name="position"/> .</summary>
            ...
            internal void CalculateGapLength(long position, long desiredLength, out long gapStart, out long gapEnd)
            {
                ...
                Debug.Assert(gapStart != -1, "Ensure that the gap bounds are not negative.");
                Debug.Assert(gapEnd != -1, "Ensure that the gap bounds are not negative.");
            }

            /// <summary>Finds the block that contains the data for the <paramref name="desiredPosition"/> .</summary>
            ...
            internal DownloadedBlock GetBlockByPosition(long desiredPosition)
            {
                ...
                lock (this.downloadedBlocksList)
                {
                    foreach (var block in this.downloadedBlocksList.Select(kvp => kvp.Value).Where(block => desiredPosition >= block.StartOffset))
                    {
                        // If we are within the block, return it. Otherwise we don't have a valid block. That shouldn't happen.
                        return (desiredPosition - block.StartOffset) < block.BlockContent.Length ? block : null;
                    }
                }
            ...
            /// <summary>Removes the specified block based on the startOffset key.</summary>
            ...
            /// <summary>Removes any downloaded blocks that are outside the current bounds (position and readAheadSize).</summary>
            ...
            internal void RemoveExtraBlocks(long position, long bufferedLength)
            {
                if (this.downloadedBlocksList.Count != 0)
                {
                    ...
                    lock (this.downloadedBlocksList)
                    {
                        for (var blockId = 0; blockId < this.downloadedBlocksList.Count; blockId++)
                        {

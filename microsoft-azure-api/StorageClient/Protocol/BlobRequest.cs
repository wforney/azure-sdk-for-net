...
namespace Microsoft.WindowsAzure.StorageClient.Protocol
{
    ...
    public static class BlobRequest
    {
            ...
            string leaseId)
        {
            ...
            var request = CreateWebRequest(uri, timeout, null);
        ...
        public static HttpWebRequest Delete(
            Uri uri, int timeout, DateTime? snapshot, DeleteSnapshotsOption deleteSnapshotsOption, string leaseId)
        {
            ...
            if (snapshot != null && deleteSnapshotsOption != DeleteSnapshotsOption.None)
            {
                throw new InvalidOperationException(
                    String.Format(SR.DeleteSnapshotsNotValidError, "deleteSnapshotsOption", "snapshot"));
            }

            AddSnapshot(builder, snapshot);
            ...
            switch (deleteSnapshotsOption)
            {
                ...
                case DeleteSnapshotsOption.IncludeSnapshots:
                    request.Headers.Add(
                        Constants.HeaderConstants.DeleteSnapshotHeader, Constants.HeaderConstants.IncludeSnapshotsValue);
                    break;
                case DeleteSnapshotsOption.DeleteSnapshotsOnly:
                    request.Headers.Add(
                        Constants.HeaderConstants.DeleteSnapshotHeader, Constants.HeaderConstants.SnapshotsOnlyValue);
                    break;
            }
        ...
        public static HttpWebRequest Get(
            Uri uri, int timeout, DateTime? snapshot, long offset, long? count, string leaseId)
        {
            var request = Get(uri, timeout, snapshot, leaseId);

            var rangeStart = offset.ToString(CultureInfo.InvariantCulture);
            var rangeEnd = string.Empty;
            if (count.HasValue)
            {
                rangeEnd = (offset + count.Value - 1).ToString(CultureInfo.InvariantCulture);
            }

            var rangeHeaderValue = string.Format(
                CultureInfo.InvariantCulture, Constants.HeaderConstants.RangeHeaderFormat, rangeStart, rangeEnd);
        ...
        /// <param name="leaseId"> The lease ID for the blob, if it has an active lease. </param>
        /// <returns> A web request to use to perform the operation. </returns>
        public static HttpWebRequest GetBlockList(
            Uri uri, int timeout, DateTime? snapshot, BlockListingFilter typesOfBlocks, string leaseId)
        {
            var builder = new UriQueryBuilder();
        ...
        {
            var builder = ContainerRequest.GetContainerUriQueryBuilder();
            ...
            if (listingContext != null)
            {
                ...
                if (listingContext.Include != BlobListingDetails.None)
                {
                    ...
                    if ((listingContext.Include & BlobListingDetails.UncommittedBlobs)
                        == BlobListingDetails.UncommittedBlobs)
                    {
                        if (!started)
                        {
                            started = true;
        ...
        /// <param name="leaseId"> The lease ID, if the blob has an active lease. </param>
        /// <param name="pageBlobSize"> For a page blob, the size of the blob. This parameter is ignored for block blobs. </param>
        /// <returns> A web request to use to perform the operation. </returns>
        public static HttpWebRequest Put(
            Uri uri, int timeout, BlobProperties properties, BlobType blobType, string leaseId, long pageBlobSize)
        {
            ...
            if (blobType == BlobType.PageBlob)
            {
                ...
                request.Headers.Add(
                    Constants.HeaderConstants.Size, pageBlobSize.ToString(NumberFormatInfo.InvariantInfo));
        ...
        /// <param name="leaseId"> The lease ID, if the blob has an active lease. </param>
        /// <returns> A web request for performing the operation. </returns>
        public static HttpWebRequest PutBlockList(Uri uri, int timeout, BlobProperties properties, string leaseId)
        {
            var builder = new UriQueryBuilder();
            builder.Add(Constants.QueryConstants.Component, "blocklist");
            ...
            Request.AddOptionalHeader(
                request, Constants.HeaderConstants.ContentLanguageHeader, properties.ContentLanguage);
            Request.AddOptionalHeader(
                request, Constants.HeaderConstants.ContentEncodingHeader, properties.ContentEncoding);
        ...
        /// <returns> A web request to use to perform the operation. </returns>
        public static HttpWebRequest SetProperties(
            Uri uri, int timeout, BlobProperties properties, string leaseId, long? newBlobSize)
        {
            var builder = new UriQueryBuilder();
            ...
            {
                request.Headers.Add(
                    Constants.HeaderConstants.Size, newBlobSize.Value.ToString(NumberFormatInfo.InvariantInfo));

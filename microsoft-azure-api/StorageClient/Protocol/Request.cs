...
namespace Microsoft.WindowsAzure.StorageClient.Protocol
{
    ...
    internal static class Request
    {
        ...
        static Request()
        {
            UserAgent = Constants.HeaderConstants.UserAgent;
        }
        ...
        internal static string ConvertDateTimeToHttpString(DateTime dateTime)
        {
            ...
            Debug.Assert(
                dateTime == DateTime.MaxValue || dateTime == DateTime.MinValue || dateTime.Kind == DateTimeKind.Utc,
        ...
        internal static HttpWebRequest CreateWebRequest(Uri uri, int timeout, UriQueryBuilder builder)
        {
            ...
            if (timeout != 0)
            {
                builder.Add("timeout", timeout.ToString(CultureInfo.InvariantCulture));
            }
        ...
        {
            var builder = GetServiceUriQueryBuilder();
        ...
        {
            var targetVersion = Constants.HeaderConstants.TargetStorageVersion;
        ...
        internal static void SignRequestForBlobAndQueue(HttpWebRequest request, Credentials credentials)
        {
            ...
            var canonicalizer = CanonicalizationStrategyFactory.GetBlobQueueFullCanonicalizationStrategy(request);
        ...
        /// <summary>
        ///   Signs requests using the SharedKeyLite authentication scheme with is used for the table storage service. Currently we only support for table.
        /// </summary>
        /// <param name="request"> The request. </param>
        /// <param name="credentials"> The credentials. </param>
        internal static void SignRequestForBlobAndQueuesSharedKeyLite(HttpWebRequest request, Credentials credentials)
        {
            ...
            // Compute the signature and add the authentication scheme
            var canonicalizer = CanonicalizationStrategyFactory.GetBlobQueueLiteCanonicalizationStrategy(request);
        ...
        internal static void SignRequestForTablesSharedKey(HttpWebRequest request, Credentials credentials)
        {
            ...
            // Compute the signature and add the authentication scheme
            var canonicalizer = CanonicalizationStrategyFactory.GetTableFullCanonicalizationStrategy(request);
...
/*
        /// <summary>
        ///   This gets the file version, but fails under partial trust with a security exception. The caller should catch the exception.
        /// </summary>
        /// <returns> File version of current assembly. </returns>
        /// <remarks>
        ///   Inlining of this fuction is explicitly disabled to support degrading gracefully under partial trust. FileVersionInfo.GetVersionInfo has a link demand assoicated with it. Therefore, we must make sure we never inline this function, otherwise the caller cannot catch the security exception associated with the link demand.
        /// </remarks>

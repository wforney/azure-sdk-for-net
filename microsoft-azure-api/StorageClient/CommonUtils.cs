...
namespace Microsoft.WindowsAzure.StorageClient
{
    ...
    /// <summary>A set of common utilities for use in verfication.</summary>
    internal static class CommonUtils
    {
        #region Methods

        /// <summary>Applies the request optimizations such as disabling buffering and 100 continue.</summary>
        ...
        /// <summary>Throw an exception indicating argument is out of range.</summary>
        ...
        /// <summary>Asserts the type of the continuation.</summary>
        /// <param name="continuationToken">The continuation token. </param>
        /// <param name="continuationType">Type of the continuation. </param>
        internal static void AssertContinuationType(
            ResultContinuation continuationToken, ResultContinuation.ContinuationType continuationType)
        {
            if (continuationToken != null)
            {
                if (
                    !(continuationToken.Type == ResultContinuation.ContinuationType.None
                      || continuationToken.Type == continuationType))
                {
                    var errorMessage = string.Format(
                        CultureInfo.CurrentCulture, SR.InvalidContinuationType, continuationToken.Type, continuationType);
                    throw new InvalidOperationException(errorMessage);
                }
            }
        }

        /// <summary>Throw an exception if the argument is out of bounds.</summary>
        ...
        internal static void AssertInBounds<T>(string paramName, T val, T min, T max) where T : IComparable
        {
            if (val.CompareTo(min) < 0)
            {
                throw new ArgumentOutOfRangeException(
                    string.Format(SR.ArgumentTooSmallError, paramName, min), paramName);
            }

            if (val.CompareTo(max) > 0)
            {
                throw new ArgumentOutOfRangeException(
                    string.Format(SR.ArgumentTooLargeError, paramName, max), paramName);
            }
        }

        /// <summary>Throw an exception if the value is null.</summary>
        /// <param name="paramName">The name of the parameter. </param>
        /// <param name="value">The value of the parameter. </param>
        ...
        /// <summary>Throws an exception if the string is empty or null.</summary>
        /// <param name="paramName">The name of the parameter. </param>
        /// <param name="value">The value of the parameter. </param>
        ...
        /// <summary>Throws if the result segment does not have more results.</summary>
        ...
        /// <summary>Asserts the segment result not empty.</summary>
        ...
        /// <summary>Combines AssertNotNullOrEmpty and AssertInBounds for convenience.</summary>
        /// <param name="paramName">The name of the parameter. </param>
        ...
        /// <summary>Look for an inner exception of type DataServiceClientException. Different versions of Sytem.Data.Services.Client have different levels of wrapping of a DataServiceClientException.</summary>
        ...
        /// <summary>When calling the Get() operation on a queue, the content of messages returned in the REST protocol are represented as Base64-encoded strings. This internal function transforms the Base64 representation into a byte array.</summary>
        /// <param name="str">The Base64-encoded string. </param>
        /// <returns>The decoded bytes. </returns>
        internal static byte[] GetContentFromBase64String(string str)
        {
            // if str is null or empty we got a message with an empty <MessageText> element
            var base64Content = string.IsNullOrEmpty(str) ? Encoding.UTF8.GetBytes(string.Empty) : Convert.FromBase64String(str);

            return base64Content;
        }

        /// <summary>Performs a 'magic enumerator' lazy segmented enumeration.</summary>
        ...
        [DebuggerNonUserCode]
        internal static IEnumerable<T> LazyEnumerateSegmented<T>(
            Func<Action<ResultSegment<T>>, TaskSequence> impl, RetryPolicy retryPolicy)
        {
            var segment = TaskImplHelper.ExecuteImplWithRetry(impl, retryPolicy);
        ...
        /// <summary>Rounds up to milliseconds.</summary>
        ...
        internal static int RoundUpToMilliseconds(this TimeSpan? timeSpan)
        {
            return (int)Math.Ceiling(timeSpan.GetValueOrDefault().TotalMilliseconds);
        }

        /// <summary>Rounds up to seconds.</summary>
        /// <param name="timeSpan">The time span. </param>
        ...
        internal static int RoundUpToSeconds(this TimeSpan? timeSpan)
        {
            return (int)Math.Ceiling(timeSpan.GetValueOrDefault().TotalSeconds);
        }

        /// <summary>Determines if a Uri requires path style addressing.</summary>
        ...
        internal static bool UsePathStyleAddressing(Uri uri)
        {
            ...
            return wellKnownDevStorePorts.Any(p => uri.Port == p);
        }
